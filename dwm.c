...
void
fullscreentoggle(const Arg *arg) {
...
setup(void)
{
	...
	const char* cmd = NULL;
	for (i = 0, cmd = autocmd[i]; cmd; ++i, cmd = autocmd[i])
		if (system(cmd) < 0 )
			die("autostartcmd failed: %s", cmd);
}
void
setviewport(void){
	long data[] = { 0, 0 };
...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
		mw = m->nmaster ? m->ww * m->mfact - 2*gappx: 0;
	else
		mw = m->ww - 2*gappx;
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			h = ( (m->wh - my) - 2*gappx * (MIN(n, m->nmaster) - i) ) / (MIN(n, m->nmaster) - i);
			resize(c, m->wx + gappx, m->wy + my + gappx, mw - (2*c->bw), h - (2*c->bw), 0);
			if (my + HEIGHT(c) < m->wh)
				my += HEIGHT(c);
		} else {
			h = ((m->wh - ty) - 2*gappx*(n-i)) / (n - i);
			resize(c, m->wx + mw + 2*gappx, m->wy + gappx + ty, m->ww - mw - (2*c->bw) - 3*gappx, h - (2*c->bw), 0);
			if (ty + HEIGHT(c) < m->wh)
				ty += HEIGHT(c) + gappx;
		}
}
